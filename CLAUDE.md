...
The husky `pre-commit` hook runs `lint-staged`, which applies `eslint --fix` then `biome format --write` to staged files. CI runs `npm run lint`, `npm run typecheck`, and `npm run format:check`.
